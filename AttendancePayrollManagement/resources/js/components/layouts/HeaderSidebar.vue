		...
		>
			...
			<v-menu offset-y>
				...
				<v-list>
					...
					<v-list-item link>
						<router-link to="/changepassword"><v-list-item-title> Change Password </v-list-item-title></router-link>
					</v-list-item>
...
<script>
	...
	export default {
		data() {
			return {
				...
				items: [
					...
					{ title: 'Attendance', 	icon: 'mdi-account-check-outline', 			to: '/admin/attendance' },
					{ title: 'Payroll', 	icon: 'mdi-file-document-outline', 			to: '/admin/payroll'},
					...
					// lloyd
					{ title: 'Request', 	icon: 'mdi-arrow-top-right', 					to: '/admin/request'},
		...
		methods: {
			...
			currentDateTime() {
                this.dateTime = moment().format('dddd, MMMM D, YYYY, h:mm:ss a');
            },
		},
		created(){
			if(localStorage.hasOwnProperty('access_token')){
				...
				this.dateTime = moment().format('dddd, MMMM D, YYYY, h:mm:ss a');
            	setInterval(() => this.currentDateTime(), 1 * 1000);
			}
			else{
				window.location.replace('/');
			}
